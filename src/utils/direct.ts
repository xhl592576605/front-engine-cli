...
export default class Direct implements IRemoteGit {
  ...
  async getOrgBranches(group: string, repos: string) {
    ...
    orgBranchApi = this.dataMatch.$matchData4String(orgBranchApi, { name: group, repo, id })
  ...
  async getUserBranches(group: string, repos: string) {
    ...
    const [repo, id] = repos.split('|')
    userBranchApi = this.dataMatch.$matchData4String(userBranchApi, { name: group, repo, id })
  ...
  async getOrgTags(group: string, repos: string) {
    ...
    const [repo, id] = repos.split('|')
    orgTagApi = this.dataMatch.$matchData4String(orgTagApi, { name: group, repo, id })
  ...
  async getUserTags(group: string, repos: string) {
    ...
    const [repo, id] = repos.split('|')
    userTagApi = this.dataMatch.$matchData4String(userTagApi, { name: group, repo, id })
